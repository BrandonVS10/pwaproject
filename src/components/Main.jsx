...
function Main() {
  ...
  useEffect(() => { 
    if (userRole === "admin") {
      fetch("https://servidorpwa.onrender.com/auth/users")
        .then((response) => {
  ...
  const registerServiceWorker = async () => {
    try {
      ...
      const response = await fetch("https://servidorpwa.onrender.com/auth/suscripcion", {
        method: "POST",
  ...
  const handleSendMessage = async (user) => {
    try {
      ...
      const response = await fetch("https://servidorpwa.onrender.com/auth/suscripcionMod", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  return (
    <div className="page-container">
      <h2 className="page-title">Zona Futbolera</h2>
      {userRole === "admin" ? (
        <div>
